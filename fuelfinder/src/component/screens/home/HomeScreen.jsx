...
const HOME_STATIONS_CACHE_FRESH_TTL_MS = 1000 * 60 * 10;
...
async function readCachedNearbyStations(
  ...
  try {
    ...
    if (!parsed || Number(parsed.savedAt || 0) <= Date.now() - maxAgeMs) {
      return null;
    }
...
function getAutoNearbyRefreshDistanceMeters(radiusMeters = 12000) {
  const normalizedRadius = Math.max(1000, Math.round(Number(radiusMeters) || 12000));
  return Math.min(
    AUTO_NEARBY_REFRESH_MAX_DISTANCE_METERS,
    Math.max(AUTO_NEARBY_REFRESH_MIN_DISTANCE_METERS, Math.round(normalizedRadius / 3))
  );
}

function hasMovedFarEnoughForNearbyRefresh(previousPoint, nextPoint, radiusMeters = 12000) {
  const movedDistanceKm = toDistanceKm(previousPoint, nextPoint);
  if (!Number.isFinite(movedDistanceKm)) return false;
  return movedDistanceKm * 1000 >= getAutoNearbyRefreshDistanceMeters(radiusMeters);
}
...
function buildNearbyRadiusPlan(radiusMeters = 12000, stationType = "fuel") {
  ...
  return [
    baseRadius,
    ...FUEL_NEARBY_RADIUS_EXPANSION_STEPS.filter((candidateRadius) => candidateRadius > baseRadius),
  ];
}
...
function buildOfflineStationsNotice(t, isElectricHome) {
  return t("homeScreen.offlineStationsNotice", {
    defaultValue: isElectricHome
      ? "Showing saved charging stations from this device until the connection comes back."
      : "Showing saved stations from this device until the connection comes back.",
  });
}

function buildOfflineStationsUnavailableMessage(t, isElectricHome) {
  return t("homeScreen.offlineStationsUnavailable", {
    defaultValue: isElectricHome
      ? "Offline and no saved charging stations are available yet. Open this screen once while online to save the list for later."
      : "Offline and no saved station list is available yet. Open this screen once while online to save the list for later.",
  });
}
...
export default function HomeScreen({ navigation, route, homeConfig = null }) {
      ...
      (isElectricHome
        ...
        : lockToCurrentCity
          ? [
              {
                id: "city",
                label: t("browseCurrentCityLabel", { defaultValue: "My city" }),
              },
            ]
          : [
            {
  ...
  const loadCityStations = useCallback(
    async (city) => {
      ...
      setStationsError("");
      try {
        const { stations: nextStations, fromCache } = await fetchDirectoryStations({
          cityId,
          limit: 220,
          stationType: preferredStationType,
          strictCity: true,
  ...
  useEffect(() => {
    ...
    if (defaultBrowseMode !== "nearby" || lockToCurrentCity) {
      return () => {
        active = false;
      };
    }
    ...
    (async () => {
      try {
        ...
        if (status !== "granted") {
          ...
          if (mounted && lockToCurrentCity) setStations([]);
  ...
  const loadNearbyStations = useCallback(async ({ forceRefresh = false } = {}) => {
    ...
    if (!basePoint) {
      ...
      if (cached?.stations?.length) {
        ...
        setStationsError("");
        setOfflineStationsNotice(buildOfflineStationsNotice(t, isElectricHome));
        return;
      }
    ...
    setStationsError("");
    try {
      const { stations: next, radiusMeters: resolvedRadiusMeters } =
      ...
      if (cached?.stations?.length) {
        ...
        setStationsError(
          isNetworkError(error)
            ? buildOfflineStationsUnavailableMessage(t, isElectricHome)
            : t("homeScreen.stations.loadFail")
        );
      }
      console.error(
  ...
  const handleBrowseModeChange = useCallback(
    async (nextMode) => {
      ...
      if (!lockToCurrentCity && !location && cityOptions[0]) {
  ...
  useEffect(() => {
    if (browseMode !== "nearby" || loadedRef.current) return;
    loadedRef.current = true;
  ...
  }, [browseMode, cityOptions, handleSelectCity, location, lockToCurrentCity, selectedCity]);

  useEffect(() => {
    if (browseMode !== "city" || !supportsCityBrowse || !location || isElectricHome) return;
    if (!lockToCurrentCity && selectedCity) return;
    ...
    (async () => {
      try {
        ...
        currentCityLookupSignatureRef.current = signature;
        if (String(selectedCity?.id || "") === String(matchedCity.id || "")) {
          return;
        }
        await handleSelectCity(matchedCity);
      } catch (_error) {
        if (active && lockToCurrentCity && !selectedCity?.id) {
          setStations([]);
          setStationsError(
            t("currentCityResolveFail", {
              defaultValue: "We could not detect your current city yet. Move a little or refresh location."
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View>
                ...
                {selectedCity ? (
                  ...
                  <Text style={styles.notice}>
                    {hasLocationPermission
                      ? t("currentCityLoading", { defaultValue: "Detecting your current city..." })
                      : t("homeScreen.location.denied")}
                  </Text>
                ) : null}
            ...
            {offlineStationsNotice ? <Text style={styles.notice}>{offlineStationsNotice}</Text> : null}
